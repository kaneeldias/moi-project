...
import {getOpportunitiesOfProject, getSurveyResponsesForProject} from "@/utils/opportunity-utils";
...
export async function GET(request: NextRequest, { params }: {params: {projectId: string}}) {
    ...
    if (!request.nextUrl.searchParams.has('entities')) {
        const projects = await getSurveyResponsesForProject(projectId);
    ...
    const projects = await getSurveyResponsesForProject(projectId, childLCs);
    return NextResponse.json(projects);
}
